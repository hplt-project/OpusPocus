...
class DecontaminateCorpusStep(CorpusStep):
    ...
    def get_valid_test_corpora(self, language: str) -> List[Path]:
        ...
        if self.valid_step is not None:
            valid_corpora = [
                Path(self.valid_step.output_dir, "{}.{}.gz".format(dset, language))
                for dset in self.valid_step.dataset_list
            ]
        if self.test_step is not None:
            test_corpora = [
                Path(self.test_step.output_dir, "{}.{}.gz".format(dset, language))
    ...
    def command(self, target_file: Path) -> None:
        ...
        for lang in self.languages:
            args = Namespace(
                **{
                    "mono": True,
                    "input_file": "{}/{}.{}.gz".format(self.input_dir, dset_name, lang),
